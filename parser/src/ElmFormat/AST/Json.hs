...
module ElmFormat.AST.Json where

import ElmFormat.Utils ((|>))

import ElmFormat.AST.Declaration
import ElmFormat.AST.Expression
import ElmFormat.AST.MapNamespace
import ElmFormat.AST.Module
import ElmFormat.AST.Pattern
import ElmFormat.AST.Variable
import ElmFormat.AST.V0_16
import Data.Maybe (mapMaybe)
import ElmFormat.Reporting.Annotation hiding (map)
...
showModule (Module _ maybeHeader _ (_, imports) body) =
    let
        header =
            maybeHeader
                |> Maybe.fromMaybe ElmFormat.AST.Module.defaultHeader
...
instance ToJSON Expr where
  showJSON (A region expr) =
      case expr of
          ...
          ElmFormat.AST.Expression.Literal (IntNum value repr) ->
              makeObj
                  [ type_ "IntLiteral"
          ...
          ElmFormat.AST.Expression.Literal (FloatNum value repr) ->
              makeObj
                  [ type_ "FloatLiteral"
                  , ("value", JSRational False $ toRational value)
                  , ("display"
          ...
          ElmFormat.AST.Expression.Literal (Boolean value) ->
            makeObj
          ...
          ElmFormat.AST.Expression.Literal (Chr chr) ->
              makeObj
                  [ type_ "CharLiteral"
          ...
          ElmFormat.AST.Expression.Literal (Str str _) ->
              makeObj
                  [ type_ "StringLiteral"
          ...
          ElmFormat.AST.Expression.Tuple exprs _ ->
              makeObj
                  [ type_ "TupleLiteral"
          ...
          ElmFormat.AST.Expression.Record base fields _ _ ->
              let
                  fieldsJSON =
                      ( "fields"
                      , makeObj $ fmap
