module ElmFormat.Parse.Module (moduleDecl, elmModule, topLevel) where
...
import ElmFormat.Parse.Declaration as Decl
  ...
  do  preModule <- option [] freshLine
      ...
      return $
        Module.Module
          ...
          ((map ElmFormat.AST.Declaration.BodyComment postImportComments) ++ decls ++ (map ElmFormat.AST.Declaration.BodyComment trailingComments))


topLevel :: IParser a -> IParser [ElmFormat.AST.Declaration.TopLevelStructure a]
topLevel entry =
  (++) <$> option [] (((\x -> [x]) <$> Decl.topLevelStructure entry))
      <*> (concat <$> many (freshDef entry))


freshDef :: IParser a -> IParser [ElmFormat.AST.Declaration.TopLevelStructure a]
freshDef entry =
    commitIf (freshLine >> (letter <|> char '_')) $
      do  comments <- freshLine
          decl <- Decl.topLevelStructure entry
          return $ (map ElmFormat.AST.Declaration.BodyComment comments) ++ [decl]
