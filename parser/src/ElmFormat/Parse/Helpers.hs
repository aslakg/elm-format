...
module ElmFormat.Parse.Helpers where
...
import qualified ElmFormat.AST.Helpers as Help
...
var :: ElmVersion -> IParser ElmFormat.AST.Variable.Ref
var elmVersion =
...
qualifiedVar :: ElmVersion -> IParser ElmFormat.AST.Variable.Ref
qualifiedVar elmVersion =
    ElmFormat.AST.Variable.VarRef
        <$> many (const <$> capVar elmVersion <*> string ".")
        <*> lowVar elmVersion


qualifiedTag :: ElmVersion -> IParser ElmFormat.AST.Variable.Ref
qualifiedTag elmVersion =
    ElmFormat.AST.Variable.TagRef
...
anyOp :: ElmVersion -> IParser ElmFormat.AST.Variable.Ref
anyOp elmVersion =
  (betwixt '`' '`' (qualifiedVar elmVersion) <?> "an infix operator like `andThen`")
  <|> (ElmFormat.AST.Variable.OpRef <$> symOp)
...
accessible :: ElmVersion -> IParser ElmFormat.AST.Expression.Expr -> IParser ElmFormat.AST.Expression.Expr
          ...
          accessible elmVersion $
            do  v <- lowVar elmVersion
                ...
                return . A.at start end $
                    -- case rootExpr of
                    --   ElmFormat.AST.Expression.VarExpr (ElmFormat.AST.Variable.VarRef name@(c:_))
                    --     | Char.isUpper c ->
                    --         ElmFormat.AST.Expression.VarExpr $ ElmFormat.AST.Variable.VarRef (name ++ '.' : v)
                    --   _ ->
                        ElmFormat.AST.Expression.Access annotatedRootExpr v
